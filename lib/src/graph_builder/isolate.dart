...
void performLayoutIsolate(dynamic input) {
  IsolateManagerFunction.customFunction<ImMap, ImMap>(
    ...
    onEvent: (controller, ImMap input) {
      ...
      final edges = <_ForceGraphEdgeData>{};
      final rand = Random();
      final rawNodes = unwrappedInput['nodes'];

      if (rawNodes is Iterable) {
        for (final n in rawNodes) {
          final node = _ForceGraphNodeData.from(n);
...
typedef _ForceGraphEdgeData = Map;

typedef _ForceGraphNodeData = Map;

extension on _ForceGraphNodeData {
  String get id => this['id'] as String;
  List<_ForceGraphEdgeData> get edges {
...
extension on _ForceGraphEdgeData {
